...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Basic/620"
  (let
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Ok/647" (makeblock 0))
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Inline_record/655"
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Extension_with_inline_record/663"
  (let
    (A =
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Float_records/678"
  (let
...
(apply (field_mut 1 (global Toploop!)) "Pattern_matching_wildcard/698"
  (let
