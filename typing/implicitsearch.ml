...
module Constraints = struct
  ...
  *)
  ...
  let register_items env
    ...
    = function
    | Sig_type (id, ty, (Trec_not | Trec_first), _vis) :: items ->
        let rec aux env id ty items =
          let env = Env.add_type ~check:false id ty env in
          match items with
          | Sig_type (id, ty, Trec_next, _vis) :: items ->
              aux env id ty items
          | _ -> env
        in
        aux env id ty items
    | Sig_module (id, pres, decl, (Trec_not | Trec_first), _vis) :: items ->
        let rec aux env id pres decl items =
          let env = Env.add_module_declaration ~check:false id pres decl env in
          match items with
          | Sig_module (id, pres, decl, Trec_next, _vis) :: items ->
              aux env id pres decl items
          | _ -> env
        in
        aux env id pres decl items
    | Sig_modtype (id, mtd, _vis) :: _ ->
  ...
  and prepare_sig_item env cstrs field =
    ...
    | Sig_type (id,decl,recst,vis) ->
        ...
        begin try
          ...
          in
          to_unify, cstrs,
          Sig_type (id,{decl with type_manifest = Some ty}, recst, vis)
        with Not_found ->
          [], cstrs, field
        end
    | Sig_module (id,pres,decl,recst,vis) ->
        let name = Ident.name id in
        begin try
          ...
          in
          to_unify, cstrs,
          Sig_module (id,pres,{decl with md_type = mty}, recst, vis)
        with Not_found ->
          [], cstrs, field
        end
  ...
  and constraint_sig_item env cstrs field =
    ...
    | Sig_module (id,_pres,decl,_recst,_vis),
  ...
  let flatten ~root cstrs =
    let flatten_cstr (n,t) =
      let id', dots = Path.flatten_with_pos n in
  ...
  let target
    ...
    let extract_constraint acc eqn =
      let id, path = Path.flatten_with_pos eqn.eq_lhs_path in
...
let remove_type_variables mark =
  ...
  let it_type_expr it ty =
    if get_level ty >= lowest_level then begin
      ...
      | _ ->
          ignore (try_mark_node mark ty);
          base_iterators.it_do_type_expr it ty;
    end
  in
  let it = {base_iterators with it_type_expr} in
  variables, it
...
let target_of_pending inst =
  ...
  let mty = match mtd.mtd_type with
                ...
                | Some mty -> mty
  in
  let basekinded (ty,_tyvar) = match get_desc ty with
    | Tconstr (_, [], _) -> true
    | Tconstr (_, (_ :: _), _) -> false
    | _ -> assert false
  in
  let basepath (ty,tyvar) = match get_desc ty with
  ...
  let variables =
    with_type_mark (fun mark ->
      let variables, it = remove_type_variables mark in
...
module Termination : sig
  ...
  let smaller env t1 t2 : [`Smaller | `Equal | `Different] =
    let rec check_ty t =
      if (try equal env true [t1] [t]; true with Equality _ -> false) then
        raise Type_is_smaller;
      iter_type_expr check_ty t
    in
    try if (try equal env true [t1] [t2]; true with Equality _ -> false)
      then `Equal
      ...
      let rec aux t2s acc = function
        ...
        | (params, t1) :: t1s ->
          let (_params', t2), t2s = list_extract
              (fun (params', _t2) -> try equal env true params params'; true with Equality _ -> false)
              t2s in
          match smaller env t1 t2 with
  ...
  let check env (uid, eqns) stack =
    begin try
      ...
      explain true Format.err_formatter (uid,eqns,eqns')
    with Not_found ->
      explain true Format.err_formatter (uid,eqns,[])
    end;
...
module Search : sig
  ...
  type 'a state =
    {
      ...
      eq_table: Ctype.equality_equation list ref Ident.Map.t;
    }
  ...
  let cleanup_equations ident eq_table =
    try
      ...
      let not_in_ident {eq_lhs_path} =
        match Path.head_opt eq_lhs_path with
        | Some h -> h <> ident
        | None -> true in
  ...
  let step0 state (path, sub_targets, candidate_mty) =
    ...
    let eq_table, env = List.fold_left
        (fun (eq_table, env) sub_target ->
          ...
          Env.add_module sub_target.target_id Mp_present sub_target.target_type env)
      ...
      (fun () ->
        ...
        with Ctype.Equality err ->
          ...
          List.iter (fun {eq_lhs; eq_rhs} ->
            ...
            !(state.eq_var);
          printf "Because:\n%!";
          let check_trace_elt = function
            | Errortrace.Diff {got; expected} ->
              printf "\t%a != %a\n%!"
                Printtyp.type_expr got.Errortrace.ty
                Printtyp.type_expr expected.Errortrace.ty;
              let rec check_expansion ty = match get_desc ty with
                | Tconstr (path,args,_)  ->
                    begin try
                ...
                | _ -> ()
              in
              check_expansion got.Errortrace.ty;
              check_expansion expected.Errortrace.ty;
              printf "\n%!"
            | _ -> ()
          in
          List.iter check_trace_elt err.Errortrace.trace;
          raise Invalid_candidate
        end;
        let _ : module_coercion =
          Includemod.modtypes ~loc:Location.none env ~mark:false candidate_mty target.target_type
        in
        ());
  ...
  let apply partial arg =
    ...
    | target :: sub_targets ->
      ...
      let env = Env.add_module_declaration ~check:false target.target_id Mp_present md arg.env in
...
let pack_implicit inst path =
  ...
  let rec translpath = function
    ...
    | Path.Papply (p1, p2) ->
        ...
        let rec loop acc mty =
          match mty with
          | Mty_functor (Named (Some param, mty_param), mty_res)
          | Mty_functor (Implicit (Some param, mty_param), mty_res) ->
              let coercion = Includemod.modtypes ~loc:Location.none env ~mark:false marg.mod_type mty_param in
              let mty_appl =
                Subst.modtype Keep
          ...
          | Mty_alias path ->
              ...
              let mty = Includemod.expand_module_alias ~strengthen:false env path in
              let acc =
  ...
  let (modl, tl') = !type_implicit_instance env modl p nl tl in
  {
    ...
    exp_type = newty (Tpackage { pack_path = p;
                                 pack_constraints = List.map2
                                   (fun n t -> Longident.flatten n, t) nl tl' });
    exp_attributes = [];
    exp_env = env;
  }
...
let generalize_implicits () =
  ...
  let need_generalization inst =
    List.exists
      (fun (ty,var) ->
         assert (get_level var <> generic_level);
         max (get_level ty) (get_level var) >= current_level)
